...
export class AsignarEncuestaComponent implements OnInit {
  ...
  fn_listarEncuesta() {
    this.ServicesProvider.get(SERVICES.LISTAR_ENCUESTAS, {}).then(
      (response) => {
  ...
  fn_asignarEncuesta(
    pk_tienda: string,
    pk_tipoEncuesta: number,
    fechaApertura: string,
    fechaCierre: string
  ) {
    fechaApertura = this.convertDate(fechaApertura);
    fechaCierre = this.convertDate(fechaCierre);
    ...
    console.log("encuesta_asignada", encuesta_asignada);
    this.ServicesProvider.post(
      SERVICES.ASIGNAR_ENCUESTA,
      encuesta_asignada,
      false,
      this.user
    ).then((response) => {
  ...
  fn_eliminarEncuesta_Asignada(id, id_tienda) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.ServicesProvider.post(
          SERVICES.ELIMINAR_ENCUESTA_ASIGNADA + id,
          pk_aplicacionEncuesta,
          false,
          this.user
        ).then((response) => {
          console.log(response);
